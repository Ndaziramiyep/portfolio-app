...
export default function About() {
  ...
  return (
    <section id="about" className={`py-16 sm:py-24 ${dark ? '' : 'bg-light'}`}>
      <div className="section-container" ref={ref}>
        ...
        >
          ...
          <h2 className={`text-3xl font-bold ${dark ? 'text-slate-light' : 'text-ink'}`}>About Me</h2>
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <motion.div
            ...
            className="space-y-5"
          >
            <p className={`text-lg leading-relaxed ${dark ? 'text-slate-muted' : 'text-ink-muted'}`}>
              Hello! I'm <span className="text-accent font-semibold">Patrick Ndaziramiye</span>, a
              passionate full stack and mobile developer based in{' '}
              <span className={dark ? 'text-slate-mid' : 'text-ink-mid'}>Kigali, Rwanda</span>. I love building things
              that live on the internet — and in your pocket.
            </p>
            <p className={`leading-relaxed ${dark ? 'text-slate-muted' : 'text-ink-muted'}`}>
            ...
            <div className="pt-4">
              <p className={`font-mono text-sm mb-4 ${dark ? 'text-slate-mid' : 'text-ink-mid'}`}>Technologies I work with:</p>
              <div className="grid grid-cols-2 gap-2">
                {technologies.map((tech) => (
                  <div key={tech} className={`flex items-center gap-2 text-sm ${dark ? 'text-slate-muted' : 'text-ink-muted'}`}>
          ...
          >
            <div className="relative mb-8">
              <div className="relative mx-auto w-56 h-56">
                ...
                <div className={`relative w-full h-full rounded-2xl border overflow-hidden flex items-center justify-center ${dark ? 'bg-navy-lighter border-navy-lighter' : 'bg-light-card border-light-border'}`}>
                  <div className="text-center">
                    <div className="w-24 h-24 rounded-full bg-accent/20 border-2 border-accent mx-auto mb-3 flex items-center justify-center">
                      <span className="text-accent font-bold text-4xl font-mono">P</span>
                    </div>
                    <p className={`font-semibold ${dark ? 'text-slate-light' : 'text-ink'}`}>Patrick Ndaziramiye</p>
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {highlights.map(({ icon: Icon, title, desc }, i) => (
                <motion.div
                  ...
                  whileHover={{ y: -4, borderColor: '#7c3aed88' }}
                  className={`rounded-xl p-4 border cursor-default transition-all duration-300 ${dark ? 'bg-navy-light border-navy-lighter' : 'bg-light-card border-light-border shadow-sm'}`}
                >
                  <Icon className="text-accent mb-2" size={20} />
                  <h3 className={`text-sm font-semibold mb-1 ${dark ? 'text-slate-light' : 'text-ink'}`}>{title}</h3>
                  <p className={`text-xs leading-relaxed ${dark ? 'text-slate-muted' : 'text-ink-muted'}`}>{desc}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
