...
const projects = [
  {
    ...
    color: 'from-violet-500/10 to-indigo-500/10', border: 'hover:border-violet-500/50',
  },
  {
    ...
    color: 'from-purple-500/10 to-violet-500/10', border: 'hover:border-purple-500/50',
  },
  {
    ...
    color: 'from-indigo-500/10 to-blue-500/10', border: 'hover:border-indigo-500/50',
  },
  {
    ...
    color: 'from-fuchsia-500/10 to-purple-500/10', border: 'hover:border-fuchsia-500/50',
  },
  {
    ...
    color: 'from-blue-500/10 to-indigo-500/10', border: 'hover:border-blue-500/50',
  },
  {
    ...
    color: 'from-violet-500/10 to-purple-500/10', border: 'hover:border-violet-500/50',
  },
]
...
function ProjectCard({ project, index, inView }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -6 }}
      className={`relative group bg-gradient-to-br ${color} rounded-2xl p-6 border glow-border ${border} transition-all duration-300 flex flex-col h-full ${dark ? 'border-navy-lighter' : 'border-light-border bg-white/60'}`}
    >
      {featured && (
        <div className="absolute top-4 right-4">
          <span className="bg-accent/20 text-accent text-xs font-mono px-2 py-1 rounded-full border border-accent/30">Featured</span>
        </div>
      )}
      <div className="flex items-start justify-between mb-4">
        <FiFolder className="text-accent" size={32} />
        <div className="flex items-center gap-1 text-xs">
          <FiStar size={12} className="text-yellow-400" />
          <span className={dark ? 'text-slate-muted' : 'text-ink-muted'}>{stars}</span>
        </div>
      </div>
      <h3 className={`font-bold text-lg mb-3 group-hover:text-accent transition-colors leading-tight ${dark ? 'text-slate-light' : 'text-ink'}`}>{title}</h3>
      <p className={`text-sm leading-relaxed mb-4 flex-1 ${dark ? 'text-slate-muted' : 'text-ink-muted'}`}>{description}</p>
      <div className="flex flex-wrap gap-2 mb-5">
        {tech.map((t) => (
          <span key={t} className={`text-xs font-mono px-2 py-1 rounded border ${dark ? 'text-slate-muted bg-navy/50 border-navy-lighter' : 'text-ink-muted bg-violet-50 border-light-border'}`}>{t}</span>
        ))}
      </div>
      <div className="flex items-center gap-4 mt-auto">
        {github && (
          <a href={github} target="_blank" rel="noopener noreferrer" className={`hover:text-accent transition-colors ${dark ? 'text-slate-muted' : 'text-ink-muted'}`} aria-label="GitHub">
            <FiGithub size={18} />
          </a>
        )}
        {live && (
          <a href={live} target="_blank" rel="noopener noreferrer" className={`hover:text-accent transition-colors ${dark ? 'text-slate-muted' : 'text-ink-muted'}`} aria-label="Live demo">
...
export default function Projects() {
  ...
  return (
    <section id="projects" className={`py-16 sm:py-24 ${dark ? '' : 'bg-light'}`}>
      <div className="section-container" ref={ref}>
        ...
        >
          ...
          <h2 className={`text-3xl font-bold ${dark ? 'text-slate-light' : 'text-ink'}`}>Projects</h2>
        ...
        <motion.p
          ...
          className={`mb-10 max-w-2xl ${dark ? 'text-slate-muted' : 'text-ink-muted'}`}
        >
            ...
            <button
              ...
              className={`font-mono text-sm px-4 py-2 rounded-full border transition-all duration-300 ${
                activeFilter === filter
                  ...
                  : dark
                    ? 'border-navy-lighter text-slate-muted hover:border-accent hover:text-accent'
                    : 'border-light-border text-ink-muted hover:border-accent hover:text-accent'
              }`}
            >
              {filter}
            </button>
        ...
        >
          <p className={`mb-4 ${dark ? 'text-slate-muted' : 'text-ink-muted'}`}>Want to see more?</p>
          <a
            href="https://github.com/patrickndaziramiye"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 border border-accent text-accent font-mono px-6 py-3 rounded hover:bg-accent/10 transition-all duration-300 hover:-translate-y-1"
          >
            <FiGithub size={18} />
            View All on GitHub
          </a>
